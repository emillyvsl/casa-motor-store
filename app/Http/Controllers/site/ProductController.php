...
class ProductController extends Controller
{
    ...
    public function review(Request $request, $product)
    {
        $request->validate([
            ...
            'rating.min' => 'A nota minima e 1.',
            'rating.max' => 'A nota maxima e 5.',
        ]);
        ...
        if (! $customer) {
            return redirect()->route('customer.login')
                ->with('error', 'Voce precisa estar logado para avaliar um produto.');
        }

        $prod = Products::where('is_active', true)->findOrFail($product);

        // Impede avaliacao duplicada
        $jaAvaliou = Reviews::where('product_id', $prod->id)
            ->where('customer_id', $customer->id)
            ->exists();

        if ($jaAvaliou) {
            return back()->with('error', 'Voce ja avaliou este produto.');
        }
        ...
        return back()->with('success', 'Avaliacao enviada com sucesso! Obrigado.');
    }
}
